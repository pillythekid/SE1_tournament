...
public class DBcontroller implements DBcontroller_IF {
	...
	int portNumber;
//CONSTRUCTOR START
	DBcontroller (int _TourneyNR, String _userName, String _password, String _dbms, String _servername, int _portnumber){
        ...
        try {        
        		...
        		try {
	        		...
	        		String query = "select gameNR from tourney where gameNR = " + TourneyNR;
	        		ResultSet result = statement.executeQuery(query);
	        		id = result.getInt("gameNR");
                } catch(Exception e) {
        			id = 0;
                }
	        if (id == TourneyNR) {
...
//REGISTER START
    public boolean registerPlayer (int _mtrnr1, int _mtrnr2){
        String updateSql =
                "UPDATE tourney"
                        +"SET `player1` = `"+_mtrnr1+"`, `player2` = `"+_mtrnr2+" WHERE `gameNR` == `"+TourneyNR+"`";
